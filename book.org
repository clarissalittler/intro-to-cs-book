...
**** Theory
     ...
     Later on, once we talk about things like assembly languages and how code executes on a computer we can talk about how the Turing machine works and how to describe algorithms for it and how to simulate it. We can talk about its role in the history of computation. 
***** Lambda calculus
***** Turing machines
...
**** Sort
     In a short chapter on sorting, we'd probably talk about how to analyze the various ways of doing sorts in terms of expected time vs. worst case time. We can also talk about things like divide & conquer strategies. 

     At the same time? One of the issues I have is that when I was first reading intro to CS books these were /really/ boring topics. I mean super boring. So what should we do instead? Is there a good way to motivate things like search and sorting?

     There's an issue here of trying to make introductory computer science and programming compelling. Projects are good for that, but is there *really* a project that motivates this? I mean *really*?

     I've been hoping that something like inventory management or something like that could be useful for motivating sorting and searching, but is that good enough? Does it really do anything useful? 

     Well, let me argue with myself for a bit. It's quite possible that what'll be motivating is just going *extra* hard in the project based structure of the text. We could really introduce all of these topics *only* in the context of discussing how to do various tasks for making the projects work. That's more intensive than I was even thinking, to be honest. I'd sorta been planning to just 
     + state the projects with their specification
     + do a bunch of topics
     + come back to the specification and tie our topics together by going through their implementation

    But maybe the chapters just need to /be/ the implementation? The problem with that approach is that it can so easily get cornered and slogged down by explaining only what's enough to get through the next part of the implementation. If we get slogged down like that, then I think the book will end up being a wreck. No, I think the instructional sections and the implementation sections need to be kept fairly separate. You should be able to just skip around and read the sections and still learn as much as possible.

    So. What's our conclusion?

    Well I don't think there's honestly an entertaining way to introduce topics like search and sorting per se. This means, though, that at some point we're just going to have to bite the bullet and just introduce some of these "eat this, it's good for you" topics just because they're sorta standard. Is that a cop out, though?

    That's what I keep struggling with. I feel like there's an organic and natural way of trying to introduce ideas that doesn't feel like a list of topics nor like an artificial tour.
...
**** Web APIs
     ...
     The reason why I want to spend time on this topic is that it's /really/ important for understanding how the modern web behaves.

     Like being able to write apps that can pull data from websites has become a completely ubiquitous task. 

     Now, actually writing android apps is way outside the scope of this text. Well, at least it most likely is. Instead we'll be focusing on coding /in/ the browser. 

     The big question, though, is are we going to explain how to run servers in JavaScript in order to explain the server side mess of /providing/ an API or do we just want to leave it opaquely and only focus on writing the code that will fetch the data? That's already not a trivial amount of code because we need to explain networking.

     How much will we need to explain in order to get through this? I mean there's the basics of HTTP requests and how they're formed as text (because we should definitely explain that it's ultimately text being sent back and forth, just text that's been organized in a standardized way)
...
***** Turing machines and classes of problems
      So one thing we /might/ want to talk about are the classic notions of difficulty classes for programming problems. Like P vs. NP etc. The problem is that do to that properly we'd probably have to spend way too much time trying to describe deterministic vs. non-deterministic turing machines. At the same time, it's just such an interesting little footnote on the history of computation. 
...
I want something like what we see in the old Sipser theory of computation. In that book they present a number of cute examples like determining if two strings are equal. The Sipser view of turing machines is very focused on accepting or rejecting strings in order to decide/recognize languages. The problem with this view is that it doesn't /really/ translate well to what I want to present in this book (and other media where I'm trying a similar thing) of having the turing machine form some kind of computation and, if it can, finish the computation by just halting and then you read out the final state of the tape. I don't /think/ it really matters to anything if we have our two end states either be "accept" or "diverge". At the same time, I guess, we *could* just keep the accept/reject/diverge three part outcome where we instead interpret accept as succesful termination, reject as an error state, and divergeing is obviously still divergeing.

We need to do something like

#+BEGIN_SRC haskell :exports none
  module TuringLang where

  import TuringCommon

  data Def = Case [(Char,Block)]
           | Loop Def
           | Call 
#+END_SRC
...
Okay, so we need a descriptor of Turing machines in code so we can translate our language to

The first file we need is going to be a bit of a common file for representing the tape and directions and things like that. So one thing we need to note is that the Tape is baaasically a list zipper where we've actually reversed the left list in the Tape representation is ordered by its closeness to the current focus. 

 
#+BEGIN_SRC haskell :exports none :tangle TuringCommon.hs
  module TuringCommon where

  data Val = Blank | Char Char

  data Dir = DLeft | DRight

  data Tape = Tape [Val] Val [Val]
...
#+BEGIN_SRC haskell :exports none :tangle Turing.hs
  module Turing where
  ...
  data TM = TM (State -> Val -> (State,Maybe Dir, Maybe Val))
