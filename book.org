...
**** Sort
    ...
    But maybe the chapters just need to /be/ the implementation? The problem with that approach is that it can so easily get cornered and slogged down by explaining only what's enough to get through the next part of the implementation. If we get slogged down like that, then I think the book will end up being a wreck. No, I think the instructional sections and the implementation sections need to be kept fairly separate. You should be able to just skip around and read the sections 
**** Search
