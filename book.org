...
If you run this code you'll see that, indeed, the functions =square.circum= and =square.area= correctly output the 
...
#+END_SRC
  but then you'd need to keep making individual accounts with this literal syntax every single time. Not only is that inconvenient, it also 
