...
**** Big or small examples?
     ...
     It occurs to me that I was a little silly up above because there's absolutely no reason to even include code in the back of the book. Code should live in a github repo that people can access. I don't think I'd want to publish with any company that *wouldn't* allow me to set up a github repo where everything can live. 

     So, yes, at some point I should probably start using this repository in order to store the code for examples. I also realize that I should probably include examples for things like machine learning as well as some of my more maker/art project oriented ideas.
** Which JavaScript?
...
* New text
** Introduction
   If you've picked up this text, you're probably interested in learning to program. There's a lot of books about programming but, I hope, this one will feel different. We'll be teaching how to code using one of the most popular programming languages in the world: JavaScript. 

   The emphasis, though, is that we'll be teaching /programming/ not just /programming in JavaScript/. Throughout this book there will be inserts and asides about how our code examples look in different languages. We'll be talking about the most general concepts whenever possible and make clear the distinction between the underlying core ideas that are the same in every language vs. the ones that are just accidents of JavaScript.

   Our emphasis on programming doesn't mean this book needs to be dry like a high school math text. There's a lot of strangeness and wonder to be found in programming, topics far beyond just the utility of building a website that stretch into brain-breaking conjectures about the fundamental nature of physics and the limits of what far flung computers can do. 

   If you're like me, you may have never seriously thought about the fact that for all the things computers /can/ do there's so many more they /can't/. I don't mean computers *now* are too slow to solve some problems but in the future they could, I mean that on a deep fundamental level there are problems computers can't solve. 

   To get a taste for this, think about 3D printers. You may have heard of 3D printers before, devices that can be used to make three-dimensional objects from a file just the same way the printers you're used to can make images on paper from a file. 

   Current 3D printers use materials like plastic layed down in thin layers to sculpt objects from the bottom up, which is really amazing but is a very /large scale/ technology. What about the microscopic scale? The quantum mechanical scale where different physics governs the behavior of atoms and their constituents? Can we ever make a 3D printer that can "print" materials atom by atom, putting everything in the exactly right quantum mechanical state? 

   If you're not sure or can't wrap your head around the question, you're in pretty good company: neither can anyone else!
...
** Control flow
   The next several topics are all related together because they involve how we /build/ a program. These parts of the language are often called "control flow" in programming and computer science because we can imagine the program chugging along down different paths through the program. 

   Why do we nee to be able to do this? Because it's the only way to write complex, general programs. For example, if we wanted a program that could perform a task five times and only five times then that's simple enough. You can just repeat the code five times. Imagine that you had a program to make a rectangle for a 3D print design. Now what if you wanted to have /multiple/ rectangles in the design? You could copy and paste the code multiple times, but that's obvious a bad idea. 

   Instead, your code needs to be /flexible/. You don't want to repeat the same concepts over and over. Imagine that you want to be flexible 
*** Making choices
**** Booleans
...
*** Functions
    Here we come upon one of the most important pieces of data and important tools for controlling a program. 
*** Repetition
**** Definite vs. indefinite
     The ability to repeat actions is something we named as a key concept in computation. 

     It shows up all the time in our day to day life. Walking 10 blocks, reading three chapters, and solving five homework problems are all obviously examples of performing the same task multiple times. These are all examples of what we call /definite/ iteration. 

     Definite in this context means "an exact number". Definite iteration is the ability to 
