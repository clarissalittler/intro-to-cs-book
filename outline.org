...
** Introduction
   ...
   To get a taste of those topics, think about 3D printers. You may have heard of 3D printers before, devices that can be used to make three-dimensional objects from a file just the same way the printers you're used to can make images on paper from a file. 

   Current 3D printers use materials like plastic layed down in thin layers to sculpt objects from the bottom up, which is really amazing but is a very /large scale/ technology. What about the microscopic scale? The quantum mechanical scale where different physics governs the behavior of atoms and their constituents? 
