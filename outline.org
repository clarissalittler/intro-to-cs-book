...
and you can see that some languages even /use/ the word "lambda" to describe these unnamed functions. 
    ...
    #+END_SRC
    which then is going to return the number /8/. We call this /substitution/ and is *the* way that computation happens in the lambda calculus. In some sense, it's the only verb (oh that's too dorky to use in this context) 
